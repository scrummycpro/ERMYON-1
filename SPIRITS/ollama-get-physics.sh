...
store_response() {
  ...
  response=$(curl -s http://localhost:11434/api/generate -d '{
    "model": "llama3",
    "prompt": "'" I need a detailed 500 word collegic paper with arguments debates and academic references and sound like an expert,the paper will be  regarding this : $prompt"'",
    "stream": false
  }' | jq -r .response)
  ...
  sqlite3 responses.db <<EOF
INSERT INTO physics (timestamp, prompt, response) VALUES ('$timestamp', '$escaped_prompt', '$escaped_response');
EOF
}
