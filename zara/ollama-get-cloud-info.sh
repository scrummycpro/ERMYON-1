...
store_response() {
  ...
  response=$(curl -s http://localhost:11434/api/generate -d '{
    "model": "llama3",
    "prompt": "'" Write a 100-word answer regarding this question in coordination with cloud-native applications' functionality, trade-offs, and essential principles, supported by examples and why this matters. Additionally, provide a 3-line rhyme that correlates with this question:$prompt"'",
    "stream": false
  }' | jq -r .response)
  ...
  sqlite3 responses.db <<EOF
INSERT INTO docker (timestamp, prompt, response) VALUES ('$timestamp', '$escaped_prompt', '$escaped_response');
EOF
}
